...
from multiview_hand_pose_cnn.nn.multiview_hand_pose_cnn import MultiViewHandPoseCNN
...
# Train function util
def train(model: MultiViewHandPoseCNN,
          data: DataLoader,
...
def evaluate(model: MultiViewHandPoseCNN,
             data: DataLoader,
             criterion: Callable[[torch.FloatTensor, torch.FloatTensor],
                                 torch.FloatTensor],
             device: torch.device,
...
def training_loop(model: MultiViewHandPoseCNN,
    ...
    for ep in range(epochs):
        ...
        start = time()
        train_loss = train(model, train_data, criterion, optimizer, device, verbose)
        end = time()
        ...
        if val_data is not None:
            start = time()
            val_loss = evaluate(model, val_data, criterion, device, verbose)
            end = time()
